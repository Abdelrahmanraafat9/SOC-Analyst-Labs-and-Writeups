# 🛡️ SOC Analyst & DFIR Lab Write-ups

Welcome to my cybersecurity lab write-ups repository! Here, I document my practical experience in **Defensive Security, Memory Forensics, Threat Hunting, and Incident Response**.

## 📌 Professional Summary & Skills
* **Focus:** Threat Triage, Digital Forensics, PCAP Analysis, Phishing Investigation.
* **Tools Used:** Volatility 2/3, Wireshark, Sysmon, Splunk, Eric Zimmerman Tools, FTK Imager.
* **Frameworks:** MITRE ATT&CK Framework, Cyber Kill Chain.

---

### 🔷 CyberDefenders Labs
| Lab Name | Category | Key Tools Used | Write-up Link |
| :--- | :--- | :--- | :--- |
| **JetBrains** | Network Forensics | Wireshark, NetworkMiner | [View Writeup](./CyberDefenders/Network-Forensics/JetBrains/) |
| **RetailBreach** | Network Forensics | Wireshark | [View Writeup](./CyberDefenders/Network-Forensics/RetailBreach/) |
| **Tomcat Takeover** | Network Forensics | Wireshark | [View Writeup](./CyberDefenders/Network-Forensics/Tomcat-Takeover/) |
| **PsExec Hunt** | Network Forensics | Wireshark | [View Writeup](./CyberDefenders/Network-Forensics/PsExec-Hunt/) |
| **PoisonedCredentials** | Network Forensics | Wireshark | [View Writeup](./CyberDefenders/Network-Forensics/PoisonedCredentials/) |
| **BlackEnergy** | Memory Forensics | Volatility 2/3 | [View Writeup](./CyberDefenders/Memory-Forensics/BlackEnergy/) |
| **Oski** | Malware Analysis | Any.Run, CyberChef | [View Writeup](./CyberDefenders/Malware-Analysis/Oski/) |
| **Red Stealer** | Threat Intelligence | VirusTotal, MalwareBazaar | [View Writeup](./CyberDefenders/Malware-Analysis/Red-Stealer/) |
| **Yellow RAT** | Threat Intelligence | VirusTotal, CyberChef | [View Writeup](./CyberDefenders/Malware-Analysis/Yellow-RAT/) |
| **PhishStrike** | Phishing Analysis | Header Analyzer, CyberChef, Any.Run | [View Writeup](./CyberDefenders/Phishing-Analysis/PhishStrike/) |

## ✉️ Connect with Me

* **LinkedIn:** [Abdelrahman Raafat](https://www.linkedin.com/in/abdelrahman-raafat-sec/)
